                    ...
                    <form wire:submit.prevent="login">
                        <label class="block">
                            ...
                            <span class="relative mt-1.5 flex">
                                <input
                                    ...
                                    placeholder="Entrez votre adresse E-mail" type="email" wire:model="email">
                        ...
                        <label class="mt-4 block">
                            ...
                            <span class="relative mt-1.5 flex">
                                <input
                                    class="form-input peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:text-slate-400/70 hover:z-10 hover:border-slate-400 focus:z-10 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    placeholder="Enter Password" type="password" wire:model="password">
                                <span class="pointer-events-none absolute flex h-full w-10 items-center justify-center text-slate-400 peer-focus:text-primary dark:text-navy-300 dark:peer-focus:text-accent">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-5 transition-colors duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @if($qrCode)
                            <div class="text-center mt-4">
                                <p>Scannez ce code QR avec votre application Google Authenticator:</p>
                                <span> {!! $qrCode !!}</span>
                            </div>
                        @endif
                        <button type="button" 
                        class="btn mt-5 w-full bg-primary font-medium text-white 
                        ...
                        wire:click="generateQrCode">Generate QR Code</button>
                        <label class="mt-4 block">
                            <span>Code OTP:</span>
                            <span class="relative mt-1.5 flex">
                                <input
                                    class="form-input peer w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 pl-9 placeholder:text-slate-400/70 hover:z-10 hover:border-slate-400 focus:z-10 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent"
                                    placeholder="Enter OTP" type="text" wire:model="otp">
                                <span class="pointer-events-none absolute flex h-full w-10 items-center justify-center text-slate-400 peer-focus:text-primary dark:text-navy-300 dark:peer-focus:text-accent">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="size-5 transition-colors duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                                    </svg>
        ...
        document.addEventListener('livewire:load', function () {
            Livewire.hook('message.processed', (message, component) => {
                if (typeof grecaptcha !== 'undefined') {
                    ...
                    grecaptcha.render('recaptcha-container'); // Recrée le reCAPTCHA
                }
            });
        });
    </script> --}}
